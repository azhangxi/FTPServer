...
// got helped from https://stackoverflow.com/questions/238603/how-can-i-get-a-files-size-in-c
int fileSizeHelper(FILE *file) {
    int prev=ftell(file);
...
bool Passivemode(){
    ...
    if (bind(pasvstatus, (const struct sockaddr*) &address, sizeof(struct sockaddr_in)) != 0){
      printf("421 Failed to bind the PASV socket");
      return false;
    }
    //get port number
                ...
                if(strcasecmp(arg[0],"cwd")==0) {
                    ...
                    if(i != 2){
                        ...
                        if (strcasecmp(arg[1], "I") == 0) {
                            ...
                            length= snprintf(buffer,1024, "%s\r\n", "500 - EBCDIC type not implemented");
                        } else if (strcasecmp(arg[1], "L") == 0) {
                            length= snprintf(buffer,1024, "%s\r\n", "500 - Local byte Byte size type not implemented");
                        } else {
                            length= snprintf(buffer,1024, "%s\r\n", "500 - TYPE not supported");
                        }
...
//                        }
                    ...
                    if(i != 2){
                        ...
                        if (strcasecmp(arg[1], "S") == 0) {
                            ...
                            length= snprintf(buffer,1024, "%s\r\n", "500 - Block Mode not implemented");
                        } else if (strcasecmp(arg[1], "C") == 0) {
                            length= snprintf(buffer,1024, "%s\r\n", "500 - Compressed Mode not implemented");
                        } else {
                    ...
                    if(i != 2){
                        ...
                        if (strcasecmp(arg[1], "F") == 0) {
                            ...
                            length= snprintf(buffer,1024, "%s\r\n", "500 - Record structure not implemented");
                        } else if (strcasecmp(arg[1], "P") == 0) {
                            length= snprintf(buffer,1024, "%s\r\n", "500 - Page structure not implemented");
                        } else {
                            length= snprintf(buffer,1024, "%s\r\n", "501 - MODE not supported");
                        }
                    }
                    ...
                    if(connectionSuccessful == false){
                        ...
                        if (listen(pasvstatus, 10) != 0){
                            printf("421 Failed to listen for PASV connections");
                            return false;
                        }
                        ...
                        char* passiveip[1024];
                        int port1 = ntohs(address.sin_port)/256;
                        int port2 = ntohs(address.sin_port)%256; 
                        int i = 0;
                ...
                } else if (strcasecmp(arg[0], "retr") == 0) {
                    if (i != 2) {
                        length = snprintf(buffer, 1024, "%s\r\n", "501 - invalid number of arguments.");
                    } else {
                        if(!passivemode){
                        ...
                        if (access(arg[1], R_OK) != -1) {
                            ...
                            int fileSize = fileSizeHelper(file);
                            char* okay = "150 File status okay. Opening data connection.\n";
                            send(clientd , okay, strlen(okay),0);

                            char newBuffer[1024];
                            bzero(newBuffer, 1024);
                            int blockSize;
                            while ((blockSize = fread(newBuffer, sizeof(char), 1024, file)) > 0) {
                                write(fd, newBuffer, blockSize);
                                bzero(newBuffer, 1024);
                            }
                            fclose(file);
                            ...
                            length = snprintf(buffer, 1024, "%s\r\n",
                                              "550 Requested action not taken. Failed to open file");
                        }
                    }
